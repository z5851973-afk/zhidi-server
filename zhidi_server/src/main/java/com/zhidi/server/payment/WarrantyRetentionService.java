...
public class WarrantyRetentionService {
	...
	@Transactional(readOnly = true)
	public Page<WarrantyRetentionResponse> listForAdmin(Pageable pageable,
			WarrantyRetentionStatus status) {
		Page<WarrantyRetention> page = status == null
			? warrantyRetentions.findAll(pageable)
			: warrantyRetentions.findByStatus(status, pageable);
		return page.map(WarrantyRetentionResponse::from);
	}
