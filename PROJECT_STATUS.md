...
- 业主资料 GET/PUT 接入和首次引导保存已在 Android 模拟器完成端到端验证：登录后读取服务端默认资料，首次引导提交资料落库，强停重启后不再回到首次引导。后端 API、Flutter API client、会话/资料状态同步、登录页和首次引导页已整理提交；剩余 app shell 路由和部分 UI 改动仍在主工作区未提交改动中。
...
- `main.dart` 中业主未登录时当前可从启动页直接进入首页；登录触发点和匿名浏览边界仍需结合产品流程核对。
...
2. 业主资料 GET/PUT 与 Android 首次引导闭环已验证；下一步继续拆分 app shell 路由、个人资料编辑/设置退出等 Flutter 未提交改动，再推进工匠账号与资料。
