...
- 2026-07-31 已按“业主直接找工人”的产品方向微调业主端候选师傅页：顶部改为工种/城市摘要、最多 3 位候选提示和“综合排序 / 经验优先 / 资料完整 / 同城服务”筛选排序胶囊；师傅条目升级为紧凑榜单式资料卡，展示姓名、工种、城市、工龄、自我介绍摘要、认证/可预约/详情看案例信息，右侧提供橙色“查看详情”和轻量“加入候选/已选”操作，更接近参考榜单视觉。页面不再展示平台价，也不伪造评分、距离、订单数等后端暂未提供的数据；已通过 `flutter test test/candidate_picker_page_test.dart`、`flutter analyze`、业主端 debug APK 构建安装和 `emulator-5554` 可视化复验。
